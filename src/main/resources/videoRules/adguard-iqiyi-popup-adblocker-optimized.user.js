...
(function () {
    ...
    // 自适应扫描间隔：空闲更慢、广告态更快
    // 目标：尽量不增加 CPU，且保证广告触发后的处理在约 2 秒窗口内完成
    var VIDEO_CLEANER_IDLE_MS = 800;
    var VIDEO_CLEANER_ACTIVE_MS = 120;
    var AD_ACTIVE_FAST_WINDOW_MS = 2000;
    // body.innerText 级别的“广告态文本”重新检测节流（避免每轮都做全量文本读取）
    var AD_STATE_RECHECK_MS = 1000;
    // 重操作频率限制：减少 removeVideoAdNodesOnce / clickSkipButtons 的调用次数
    var AD_REMOVE_EVERY_MS = 250;
    var AD_CLICK_EVERY_MS = 350;
    // 多标签页保护：页面不可见时降频到更慢，避免同时打开多个页面造成累计 CPU 压力
    var VIDEO_CLEANER_HIDDEN_MS = 1800;
    // 浏览器窗口失焦（但页面未 hidden）时进一步降频
    var VIDEO_CLEANER_BLUR_MS = 2200;
    // 熔断保护：短时间连续异常超过阈值后，自动停用视频清理循环，防止页面卡死
    var CLEANER_MAX_ERRORS = 6;
    var CLEANER_ERROR_WINDOW_MS = 10000;
    // 爱奇艺域名关键字
    ...
    // shouldTreatAsIqiyiAdState() 使用 body 文本判断，属于“重操作”，这里做节流
    var lastAdStateCheckAt = 0;
    var cleanerStarted = false;
    var cleanerDisabled = false;
    var cleanerErrorCount = 0;
    var cleanerErrorWindowStart = 0;
    ...
    function installFetchAndXhrAdBlocker() {
        ...
        if (typeof originFetch === "function" && !originFetch.__pbWrapped) {
            ...
            window.fetch.__pbWrapped = true;
        }

        var originOpen = XMLHttpRequest.prototype.open;
        var originSend = XMLHttpRequest.prototype.send;
        if (!originOpen.__pbWrapped) {
            XMLHttpRequest.prototype.open = function (method, url) {
                // 把 url 缓存在 XHR 实例上，便于 send 阶段判断
                this.__pbUrl = url;
                return originOpen.apply(this, arguments);
            };
            XMLHttpRequest.prototype.open.__pbWrapped = true;
        }
        if (!originSend.__pbWrapped) {
            ...
            XMLHttpRequest.prototype.send.__pbWrapped = true;
        }
    }
    ...
    function clickSkipButtonsIfExists(primaryVideoOpt) {
        // 扫描页面里“疑似跳过/关闭广告”的按钮并尝试点击
        // 可选参数 primaryVideoOpt 用于复用主播放器对象，减少重复 query/boundingRect 计算
        var primary = primaryVideoOpt || getPrimaryVideoElement();
        if (primary && primary.paused) {
            return;
        }
        // 为了控制 CPU：仅扫描 button（原实现包含 span/div，查询范围更大）
    ...
    function trySkipPrerollByVideoState(primaryVideoOpt) {
        // 视频态跳尾：当检测到“广告态文本”或“父容器广告文案”时，静音+倍速并 seek 到广告尾段
        // 可选参数 primaryVideoOpt 用于复用主播放器，减少重复 query/boundingRect 计算
        var video = primaryVideoOpt || getPrimaryVideoElement();
        if (!video) {
            return;
        }
        if (video.paused) {
        ...
        // shouldTreatAsIqiyiAdState() 会读取 body.innerText，属于重操作，所以节流执行
        var nowTsForAdState = now();
        var adState = false;
        if (nowTsForAdState - lastAdStateCheckAt >= AD_STATE_RECHECK_MS) {
            adState = shouldTreatAsIqiyiAdState();
            lastAdStateCheckAt = nowTsForAdState;
        }
        var parentText = "";
        // 使用 textContent：避免 innerText 的布局计算成本（降低 CPU）
        if (video.parentElement && video.parentElement.textContent) {
            parentText = video.parentElement.textContent;
        }
        var maybeAd = adState ||
            parentText.indexOf("VIP可关闭广告") >= 0 ||
            parentText.indexOf("精彩即将开始") >= 0;
        if (maybeAd) {
            var nowTs = nowTsForAdState;
            // 重复触发保护：同一个广告段在锁定期内不再重复执行 seek（避免“广告重复播放”）
    ...
    function onCleanerError(error) {
        // 防崩熔断：短时间内异常过多则停用视频清理循环，避免反复报错拖垮页面
        var ts = now();
        if (!cleanerErrorWindowStart || ts - cleanerErrorWindowStart > CLEANER_ERROR_WINDOW_MS) {
            cleanerErrorWindowStart = ts;
            cleanerErrorCount = 0;
        }
        cleanerErrorCount += 1;
        if (cleanerErrorCount >= CLEANER_MAX_ERRORS) {
            cleanerDisabled = true;
            renderToast("[Popup Blocker] 已触发防崩熔断，自动停止视频广告清理以保护页面稳定");
        }
        try {
            // 保留最小日志，便于你本地排查；不抛出，避免影响页面主流程
            if (window.console && typeof window.console.warn === "function") {
                window.console.warn("[Popup Blocker] cleaner error:", error);
            }
        } catch (e) {
            // ignore
        }
    ...
    function installVideoAdCleaner() {
        // 启用视频广告处理（自适应节奏）：
        // 1) 空闲时降低轮询频率，避免“时时刻刻扫描”导致 CPU 上升
        // 2) 一旦检测到“跳过/关闭广告”等迹象，进入约 2 秒快速窗口做重操作
        // 3) removeVideoAdNodesOnce / clickSkipButtonsIfExists 做频率限制，避免重复 DOM 大范围查询
        // 4) 发生连续异常时自动熔断停用，优先保证页面不崩
        if (cleanerStarted) {
            return;
        }
        cleanerStarted = true;
        ...
        function loop() {
            ...
            var nowTs = now();
            // 后台标签页直接走慢速心跳，不做重操作（减少多页面并发开销）
            if (document.hidden) {
                window.setTimeout(loop, VIDEO_CLEANER_HIDDEN_MS);
                return;
            }
            try {
                var primary = getPrimaryVideoElement();
                if (primary && !primary.paused) {
                    // 快速窗口：用父容器文本做轻量判断，尽快把处理节奏切到“2 秒窗口内”
                    var parentText = "";
                    if (primary.parentElement && primary.parentElement.textContent) {
                        parentText = primary.parentElement.textContent;
                    }
                    var maybeAdQuick = parentText.indexOf("跳过广告") >= 0 ||
                        parentText.indexOf("关闭广告") >= 0 ||
                        parentText.indexOf("VIP可关闭广告") >= 0 ||
                        parentText.indexOf("精彩即将开始") >= 0;

                    if (maybeAdQuick) {
                        adFastUntil = nowTs + AD_ACTIVE_FAST_WINDOW_MS;
                    }

                    // seek/倍速逻辑会在“广告态”才执行；body.innerText 在 trySkip 内部也做了节流
                    trySkipPrerollByVideoState(primary);

                    // 只有在 2 秒快速窗口内才做重操作（DOM 清理/点击按钮）
                    if (nowTs < adFastUntil) {
                        if (nowTs - lastRemoveAt >= AD_REMOVE_EVERY_MS) {
                            removeVideoAdNodesOnce();
                            lastRemoveAt = nowTs;
                        }
                        if (nowTs - lastClickAt >= AD_CLICK_EVERY_MS) {
                            clickSkipButtonsIfExists(primary);
                            lastClickAt = nowTs;
                        }
                    }
                }
            ...
            var pageBlurred = typeof document.hasFocus === "function" ? !document.hasFocus() : false;
            var delay = pageBlurred ?
                VIDEO_CLEANER_BLUR_MS :
                (nowTs < adFastUntil ? VIDEO_CLEANER_ACTIVE_MS : VIDEO_CLEANER_IDLE_MS);
            window.setTimeout(loop, delay);
        }

        window.setTimeout(loop, VIDEO_CLEANER_IDLE_MS);
    }
